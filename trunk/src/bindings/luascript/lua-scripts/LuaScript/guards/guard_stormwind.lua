-- Mango Style guard_stormwind script
...
function Guard_Stormwind_GossipSelect(player, _Creature, sender, action)
	--Serving guard_stormwind/main menu
	if sender == GOSSIP_SENDER_MAIN then
		Guard_Stormwind_SendDefaultMenu(player, _Creature, action)

	--Came from the second menu already
	elseif sender == GOSSIP_SENDER_SEC_CLASSTRAIN then
		Guard_Stormwind_SendClassTrainerMenu(player, _Creature, action)

	elseif sender == GOSSIP_SENDER_SEC_PROFTRAIN then
		Guard_Stormwind_SendProfTrainerMenu(player, _Creature, action)
...
-- React on selections from the main menu
function Guard_Stormwind_SendDefaultMenu(player, _Creature,  action)
	if action == GOSSIP_ACTION_INFO_DEF + 1 then

		player:SEND_POI(-8811.46, 667.46, 6, 6, 0, "Stormwind Auction House") 	-- Puts an icon on the player his minimap
		player:SEND_GOSSIP_MENU(3834,_Creature:GetGUID()) 				-- Sends the location 'info' (in text)
...
-- Sends the Battle Master menu
function Guard_Stormwind_SendBattleMasterMenu(player, _Creature, action)
	if action == GOSSIP_ACTION_INFO_DEF + 1 then
...
function Guard_Stormwind_SendClassTrainerMenu(player, _Creature, action)
	if action == GOSSIP_ACTION_INFO_DEF + 1 then
...
function Guard_Stormwind_SendProfTrainerMenu(player, _Creature, action)
	if action == GOSSIP_ACTION_INFO_DEF + 1 then
